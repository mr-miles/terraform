...
`,
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			diags = diags.Append(test.Diag) // to normalize it into a tfdiag.Diagnostic
			diag := diags[0]
			got := strings.TrimSpace(DiagnosticPlain(diag, sources, colorize, 40))
			want := strings.TrimSpace(test.Want)
			if got != want {
				t.Errorf("wrong result\ngot:\n%s\n\nwant:\n%s\n\n", got, want)
			}
...
...
`
	output := DiagnosticPlain(diags[0], sources, color, 80)
...
eventually make it onto multiple lines. THE END
`
	output := DiagnosticPlain(diags[0], nil, color, 76)
